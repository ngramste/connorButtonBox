...
const int numLeds = 6;
const bool invertButtons = true;

const int buttonIndecies [numButtons] = {5, 4, 3, 2, 1, 0, 23, 22, 21, 20, 13, 6};
const int ledIndecies [numLeds] = {12, 11, 10, 9, 8, 7};
...
void setup() {
  ...
  for (int i=0; i<numButtons; i++) {
    pinMode(buttonIndecies[i], INPUT_PULLUP);
  }
  for (int i=0; i<numLeds; i++) {
    pinMode(ledIndecies[i], OUTPUT);
  }
...
void loop() {
  ...
  for (int i=0; i<numButtons; i++) {
    if (digitalRead(buttonIndecies[i])) {
    ...
    Joystick.button(i + 1, (invertButtons?!allButtons[i]:allButtons[i]));

    for (int i=0; i<numLeds; i++) {
      digitalWrite(ledIndecies[i], (invertButtons?!allButtons[i]:allButtons[i]));
    }
  }
